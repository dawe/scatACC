...
tn_list = list(set(dbc_A.values())) # ['tn5', 'tnH']
...
def get_options():
    parser = argparse.ArgumentParser(prog='demux_getseq.py')
...
def demux():
    ...
    filenames = [f'{options.prefix}_{a}_{r}.fastq.gz' for a in tn_list for r in suffix]
    un_filenames = []
    ...
    for reads in read_iterator:
        ...
        if bc_dist[amin] <= _BC_MAXDIST:
            ...
            use_tn = dbc_A[bc1]
            fname1 = f'{options.prefix}_{use_tn}_R1.fastq.gz'
            fname2 = f'{options.prefix}_{use_tn}_R2.fastq.gz'
            # found
